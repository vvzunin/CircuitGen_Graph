...
public:
  ...
  basic_string_view(const Char *s) :
      data_(s), size_(detail::const_check(std::is_same<Char, char>::value &&
...
public:
  ...
  explicit iterator_buffer(std::back_insert_iterator<Container> out,
                           size_t = 0) : iterator_buffer(get_container(out)) {}
...
struct arg_mapper {
  ...
  FMT_CONSTEXPR FMT_INLINE auto
  map(const T &named_arg) -> decltype(this->map(named_arg.value)) {
