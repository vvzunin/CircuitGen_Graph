...
TEST(GraphUtTests, GetLogicOperTest){
    std::pair<std::string_view, int32_t> funcName = GraphUtils::getLogicOperation("and");
    std::pair<std::string_view, int32_t> actualName = {"and", 4};
    ASSERT_EQ(funcName, actualName);
}

TEST(GraphUtTests, GetLogicDeathTest){
    ASSERT_DEATH (GraphUtils::getLogicOperation("nonexistentName"), "value not found in operations' container");
}

TEST(GraphUtTests, GetOperationsGatesTest){
    std::pair<std::vector<bool>, std::vector<Gates>> funcOperations = GraphUtils::getLogicOperationsWithGates();
    ...
    static std::vector<bool> logicValues = {false, false, false, false, false, false, true, true};
    std::pair<std::vector<bool>, std::vector<Gates>> actualOperations = {logicValues, logicElements};
...
TEST(GraphUtTests, fromOperToNameDeathTest){
    ASSERT_DEATH (GraphUtils::fromOperationsToName("nonexistentName"), "name not found in operations' container");
}
