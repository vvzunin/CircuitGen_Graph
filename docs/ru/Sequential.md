...
Порядок букв в имени типа важен. Так, для сигнала `nffre` (_flip-flop with negedge clock signal, reset sync signal and enable signal_) после передачи тактового сигнала и сигнала данных нужно передать сигнал `EN`, затем `RST`. Такой порядок соответствует перегрузкам `addSequential` и внутреннему порядку входов (`data`, `clk`, `en`, `rst/clr`, `set`). В случае, если, скажем, указать тип, ожидающий больше/меньше сигналов, чем было передано, то в поток ошибок (или в поток для логирования ошибок, если  включено логирование) будет выведено предупреждение о том, что сигнал для определенного флага не был получен (с указанием флага).
